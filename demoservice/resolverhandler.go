			...
			if paDomain[0] != "demo-service" {
				...
				if len(ccy) == 0 {
					...
					if ssn.Log(err, "resolverHandler: Verify signature") {
						...
						signerVerified, err := ssnclient.VerifySigner(req.Signer, req.Ssn_account, ssnAPI)
						if ssn.Log(err, "resolverHandler: Verify signature") {
							resolve.Title = "Requestor is not authorized to view payment details"
							resolve.Status = 403
							...
							paAddress := strings.Split(pa[0], ":")
							fmt.Println(paAddress)
							// This response only returns currency codes accepted not amounts
							if len(paAddress) == 1 {
								...
								// This response returns a currency and amount as specified by the payment address provided the currency has a trustline
							} else if len(paAddress) == 3 {
								ccySliced := []ssnclient.ResolverPaymentDetails{}
								for i := range ccy {
									if strings.ToUpper(paAddress[2]) == ccy[i].Asset_code {
										ccySliced = append(ccySliced, ssnclient.ResolverPaymentDetails{
											Amount:     paAddress[1],
											Asset_code: strings.ToUpper(paAddress[2]),
										})
									}
								}
								resolve.Network_address = serviceSK.Address()
